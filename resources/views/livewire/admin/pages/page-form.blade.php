            ...
            <form wire:submit.prevent="updateSocial" class="flex-none flex flex-col justify-between">
                <div class="overflow-y-auto">
                                ...
                                <div class="card-body flex">
                                    <div class="flex flex-wrap">
                                        <div class="input-group w-1/2 pr-2">
                                            ...
                                            <input wire:model="socials.{{ $index }}.name" type="text" id="name"
                                                maxlength="80">
                                        </div>
                                        <div class="input-group w-1/2 pl-2">
                                            <label for="icon">Icon <span class="font-light italic">(fontawesome
                                                    6)</span></label>
                                            <input wire:model="socials.{{ $index }}.icon" type="text" id="icon"
                                                maxlength="80">
                                        </div>
                                        <div class="input-group w-1/2 pr-2">
                                            <label for="color">Color <span
                                                    class="font-light italic">(hexcode)</span></label>
                                            <input wire:model="socials.{{ $index }}.color" type="text" id="color"
                                                maxlength="80">
                                        </div>
                                        <div class="input-group w-1/2 pl-2">
                                            <label for="link">Link</label>
                                            <input wire:model="socials.{{ $index }}.link" type="text" id="link"
                                                maxlength="80">
                                        </div>
                                        <div class="input-group w-1/2">
                                            <label for="display">Display</label>
                                            <input class="w-8 h-8" type="checkbox"
                                                wire:model="socials.{{ $index }}.display">
                                        </div>
                                    </div>
                                    <div class="m-2">

                                        <a class="cursor-pointer flex justify-center align-middle pl-4"
                    ...
                    @elseif($type == 'Comment')
                        ...
                        @endif
                        <button class="" type="button" wire:click="updateComment"
                    ...
                    @elseif($type == 'Meta')
                        <div>
                                    ...
                                    <div class="card-body flex">
                                        <div class="input-group w-full">
                                            @if ($meta['name'] == 'Twitter Site')
                                                <label for="content">{{ $meta['name'] }} <span
                                                        class=" inline-block text-xs font-light italic"> ex:
                                                        @john_doe</span></label>
                                            @else
                                                <label for="content">{{ $meta['name'] }}</label>
                                            @endif
                                            @if ($meta['name'] == 'Description')
                                                <textarea class="w-full" wire:model="metas.{{ $index }}.content"
                                                    name="description" rows="4"></textarea>
                                            @else
                                                <input class="w-full" wire:model="metas.{{ $index }}.content"
                            ...
                            @endif
                            <button class="" type="button" wire:click="updateMeta"
                                wire:loading.attr="disabled">Update</button>
                        </div>
                    @endif
