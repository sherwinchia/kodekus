<div class="data-table">
  <div class="top">
    <div class="flex justify-between mb-2">
      ...
      <a href="{{ route('admin.guards.create') }}" role="button">Create</a>
    </div>
  </div>
  <div class="bottom">
        ...
        <tr>
          <td class="px-6 py-4 whitespace-no-wrap border-b">
            <div class="flex items-center">
              <div>
                <div class="text-sm leading-5 text-gray-800">{{ $guard->id }}</div>
              </div>
            </div>
          </td>
          <td class="non-id">
          ...
          <td class="non-id">
            <div class="flex justify-center text-gray-600">
              <a class="mx-1 text-lg" role="button" href="{{ route('admin.guards.edit', $guard->id) }}">
                <i class="fas fa-edit"></i>
              </a>
              <a class="mx-1 text-lg" role="button"
                wire:click="$emitTo('admin.partials.delete-modal-component', 'onTrashIcon' ,{{ $guard->id }}, 'guard')">
    ...
    <div class="sm:flex-1 sm:flex sm:items-center sm:justify-between mt-4 work-sans">
      <div>
        <p class="text-sm leading-5">
          Showing
          <span class="font-medium">{{ $guards->firstItem() }}</span>
          to
          <span class="font-medium">{{ $guards->lastItem() }}</span>
          of
          <span class="font-medium">{{ $guards->total() }}</span>
          results
        </p>
      </div>
