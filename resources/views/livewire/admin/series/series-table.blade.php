        ...
        <tr>
          <td class="px-6 py-4 whitespace-no-wrap border-b">
            <div class="flex items-center">
              <div>
                <div class="text-sm leading-5 text-gray-800">{{ $serie->id }}</div>
              </div>
            </div>
          </td>
          <td class="non-id">
          ...
          <td class="non-id">
            <div class="flex justify-center text-gray-600">
              <a class="mx-1 text-lg" role="button" href="{{ route('admin.series.edit', $serie->id) }}">
                <i class="fas fa-edit"></i>
              </a>
              <a class="mx-1 text-lg" role="button" wire:click="$emit('onTrashIcon',{{ $serie->id }})">
...
<script>
  function tableComponents(){
      return{
        ...
        listen() {
          window.livewire.on('onTrashIcon', seriesId => {
            this.id = seriesId;
